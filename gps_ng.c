...
//#define PRINT_ACQUIRE_SQUETCH  150000
...
unsigned samples_per_ms;
unsigned if_cycles_per_ms;
unsigned samples_for_acquire;
unsigned acquire_bitmap_size_u32;
unsigned code_offset_in_ms;
unsigned ms_for_acquire = 4;
unsigned int acquire_min_power;
unsigned int track_unlock_power;
unsigned int lock_lost_power;
...
struct Space_vehicle {
  ...
  uint_32 acquire_max_power;
  uint_8  acquire_max_band;
  uint_32 acquire_max_offset;

  
  uint_8  track_band;
  uint_32 track_offset; 
  uint_32 track_power[3][3];
  uint_32 track_max_power;
  uint_8  track_max_band;
  uint_32 track_max_offset;

  uint_32 lock_filtered_power;
  uint_32 lock_phase_nco_sine;
  uint_32 lock_phase_nco_cosine;
  int_32  lock_phase_nco_step;

  uint_32 lock_code_nco_step;
  uint_32 lock_code_nco_filter;
  uint_32 lock_code_nco;
  int_32  lock_code_nco_trend;
	
  int_32 lock_early_sine_total;
  int_32 lock_early_cosine_total;
  int_32 lock_prompt_sine_total;
  int_32 lock_prompt_cosine_total;
  int_32 lock_late_sine_total;
  int_32 lock_late_cosine_total;
  
  int_32 lock_early_power;
  int_32 lock_early_power_not_reset;
  int_32 lock_prompt_sine_power;
  int_32 lock_prompt_cosine_power;
  int_32 lock_prompt_power;
  int_32 lock_late_power;
  int_32 lock_late_power_not_reset;
  uint_8 lock_last_angle;
  int_32 lock_delta_filtered;
  int_32 lock_angle_filtered;
  uint_8 lock_ms_of_bit;
  int_32 lock_ms_of_frame;
  uint_8 lock_last_bit;
  FILE *bits_file; 
  FILE *nav_file;
  ...
  uint_32 nav_seq;
  uint_8 nav_frame_synced;
  uint_32 nav_frame_of_week;

  /* Last 32 bits of data received*/
  uint_8 nav_part_in_bit;
  uint_8 nav_subframe_in_frame;
  uint_32 nav_new_subframe;
  
  /* A count of where we are upto in the subframe (-1 means unsynced) */
  int_32          nav_valid_bits;
  int_32          nav_bit_errors;

  /* Uncoming, incomplete frame */
  uint_32   nav_subframe[10];

  /* Complete NAV data frames (only 1 to 5 is used, zero is empty) */
  uint_8  nav_valid_frame[6];
  uint_32 nav_frames[6][10];  
...
static void attempt_solution(struct Snapshot *s) {
	...
	for(q = 0; q < sv_count; q++) {
		...
		space_vehicles[sv].time_raw   = s->entries[e].nav_frame_of_week * 6000 + s->entries[e].lock_ms_of_frame
...
static void nav_save_frame(struct Space_vehicle *sv) {
  ...
  if(sv->nav_subframe[0] & 1)
    unflipped[0] = sv->nav_subframe[0];
  else
    unflipped[0] = ~sv->nav_subframe[0];

  /* The next 9 depend on the value of bit 0 of the previous subframe */
  for(i = 1; i < 10; i++) {
    if(sv->nav_subframe[i-1] & 1)
      unflipped[i] = ~sv->nav_subframe[i];
    else
      unflipped[i] = sv->nav_subframe[i];   
  }
  ...
  sv->nav_frame_of_week = (handover_word >> 13) & 0x1FFFF; 
  sv->lock_ms_of_frame = 0;

  char buf[100];
  time_t timestamp;
  struct tm  ts;

  timestamp = TIME_EPOCH + sv->nav_time.week_no * 604800 + sv->nav_frame_of_week * 6;
  ts = *localtime(&timestamp);
  strftime(buf, sizeof(buf), "%a %Y-%m-%d %H:%M:%S %Z", &ts);

  
  printf("Time of next frame %i %s :",sv->nav_frame_of_week, buf);
  ...
  if(frame_type > 0 && frame_type < 6) {
	  ...
	  if(sv->nav_file != NULL) {
		  fseek(sv->nav_file, sizeof(sv->nav_subframe)*(frame_type-1), SEEK_SET);
		  fwrite(sv->nav_subframe,sizeof(sv->nav_subframe),1,sv->nav_file);
		  fflush(sv->nav_file);
	  }
  }
	  
  if(frame_type == 1) {
    sv->nav_valid_frame[1] = 1;
	  for(i = 0; i < 10; i++)
         sv->nav_frames[1][i] = unflipped[i];
  
      sv->nav_time.week_no              = join_bits_u(0,           0, 0, sv->nav_frames[1][2], 20,10);
      ...
      sv->nav_time.user_range_accuracy  = join_bits_u(0,           0, 0, sv->nav_frames[1][2], 14, 4);
      sv->nav_time.health               = join_bits_u(0,           0, 0, sv->nav_frames[1][2],  8, 6);
      sv->nav_time.group_delay     = join_bits_s(0,           0, 0, sv->nav_frames[1][6],  6, 8) * pow(2.0, -31.0);
      sv->nav_time.issue_of_data  = join_bits_u(sv->nav_frames[1][2],6, 2, sv->nav_frames[1][7], 22, 8);
      sv->nav_time.reference_time = join_bits_u(0,           0, 0, sv->nav_frames[1][7],  6,16) * pow(2.0, 4.0);
      sv->nav_time.correction_f2  = join_bits_s(0,           0, 0, sv->nav_frames[1][8], 22, 8) * pow(2.0, -55.0);
      sv->nav_time.correction_f1  = join_bits_s(0,           0, 0, sv->nav_frames[1][8],  6,16) * pow(2.0, -43.0);
      sv->nav_time.correction_f0  = join_bits_s(0,           0, 0, sv->nav_frames[1][9],  8,22) * pow(2.0, -31.0);
      debug_print_time(sv);
    ...
    if(frame_type == 2) {
	  sv->nav_valid_frame[2] = 1;
      for(i = 0; i < 10; i++)
        sv->nav_frames[2][i] = unflipped[i];
    } else {
	  sv->nav_valid_frame[3] = 1;
      for(i = 0; i < 10; i++)
        sv->nav_frames[3][i] = unflipped[i];
    }
    ...
    **************************************************/
    iode2               = join_bits_u(0,            0, 0, sv->nav_frames[2][2], 22, 8);
    iode3               = join_bits_u(0,            0, 0, sv->nav_frames[3][9], 22, 8);
    if(iode2 == iode3 && iode2 != sv->nav_orbit.iode) {
      ...
      ****************************************************/
      ...
      sv->nav_orbit.Crs                      = join_bits_s(0,            0, 0, sv->nav_frames[2][2],  6,16) * pow(2.0,-5.0);
      sv->nav_orbit.delta_n                  = join_bits_s(0,            0, 0, sv->nav_frames[2][3], 14,16) * pow(2.0,-43.0) * PI;
      sv->nav_orbit.mean_motion_at_ephemeris = join_bits_s(sv->nav_frames[2][3], 6, 8, sv->nav_frames[2][4],  6,24) * pow(2.0,-31.0) * PI;
      sv->nav_orbit.Cuc                      = join_bits_s(0,            0, 0, sv->nav_frames[2][5], 14,16) * pow(2.0,-29.0);
      sv->nav_orbit.e                        = join_bits_u(sv->nav_frames[2][5], 6, 8, sv->nav_frames[2][6],  6,24) * pow(2.0,-33.0);
      sv->nav_orbit.Cus                      = join_bits_s(0,            0, 0, sv->nav_frames[2][7], 14,16) * pow(2.0,-29.0);
      sv->nav_orbit.sqrt_A                   = join_bits_u(sv->nav_frames[2][7], 6, 8, sv->nav_frames[2][8],  6,24) * pow(2.0,-19.0);
      sv->nav_orbit.time_of_ephemeris        = join_bits_u(0,            0, 0, sv->nav_frames[2][9], 14,16) * 16.0;
      sv->nav_orbit.fit_flag                 = join_bits_u(0,            0, 0, sv->nav_frames[2][9], 13, 1);
      sv->nav_orbit.aodo                     = join_bits_u(0,            0, 0, sv->nav_frames[2][9],  8, 6);

      /******** From FRAME 3 *******/
      sv->nav_orbit.Cic                      = join_bits_s(0,            0, 0, sv->nav_frames[3][2], 14,16) * pow(2.0,-29.0);
      sv->nav_orbit.omega_0                  = join_bits_s(sv->nav_frames[3][2], 6, 8, sv->nav_frames[3][3],  6,24) * pow(2.0,-31.0) * PI;
      sv->nav_orbit.Cis                      = join_bits_s(0,            0, 0, sv->nav_frames[3][4], 14,16) * pow(2.0,-29.0);
      sv->nav_orbit.inclination_at_ephemeris = join_bits_s(sv->nav_frames[3][4], 6, 8, sv->nav_frames[3][5],  6,24) * pow(2.0,-31.0) * PI;
      sv->nav_orbit.Crc                      = join_bits_s(0,            0, 0, sv->nav_frames[3][6], 14,16) * pow(2.0,-5.0);
      sv->nav_orbit.w                        = join_bits_s(sv->nav_frames[3][6], 6, 8, sv->nav_frames[3][7],  6,24) * pow(2.0,-31.0) * PI;
      sv->nav_orbit.omega_dot                = join_bits_s(0,            0, 0, sv->nav_frames[3][8],  6,24) * pow(2.0,-43.0) * PI;
      sv->nav_orbit.idot                     = join_bits_s(0,            0, 0, sv->nav_frames[3][9],  8,14) * pow(2.0,-43.0) * PI;
      sv->nav_orbit.orbit_valid = 1;
      debug_print_orbit(sv);
    }
  } else if(frame_type == 4) {
	  sv->nav_valid_frame[4] = 1;
    /* Frame not used - holds almanac data */
  } else if(frame_type == 5) {
	  sv->nav_valid_frame[5] = 1;
    /* Frame not used - holds almanac data */
  } else {
...
static int nav_read_in_cached_data(struct Space_vehicle *sv) {
	...
	while(fread(sv->nav_subframe,40,1,f) == 1) {
...
static int nav_test_telemetry(struct Space_vehicle *sv) {
	unsigned int temp = sv->nav_new_subframe & 0x3FC00000;
...
static int nav_valid_subframe(struct Space_vehicle *sv) {
	if(!nav_test_parity(sv->nav_new_subframe)) {
#if SHOW_NAV_FRAMING
...
#endif
		sv->nav_subframe[sv->nav_subframe_in_frame] = sv->nav_new_subframe;
		sv->nav_frame_synced      = 1;
		sv->nav_subframe_in_frame = 1;
		sv->lock_ms_of_frame      = 600;
	} else {
		if(sv->nav_subframe_in_frame == 0) {
			if(!nav_test_telemetry(sv)) {
#if SHOW_NAV_FRAMING
				printf("%02i:%5i: Telemetry fail 0x%08X\n", sv->id, sv->nav_seq, sv->nav_new_subframe );
#endif
				sv->nav_frame_synced = 0;
				return 0;
			}
#if SHOW_NAV_FRAMING
...
#if SHOW_NAV_FRAMING
			printf("%02i:%5i:    Subframe %i\n", sv->nav_seq, sv->id, sv->nav_subframe_in_frame);
#endif
			sv->nav_subframe[sv->nav_subframe_in_frame] = sv->nav_new_subframe;
			if(sv->nav_subframe_in_frame  == 9) {
				nav_save_frame(sv);
				sv->nav_subframe_in_frame = 0;
			} else {
				sv->nav_subframe_in_frame++;
			}
		}
	}
	return 1;
...
static void nav_new_bit(struct Space_vehicle *sv, uint_8 s) {
	...
	sv->nav_new_subframe <<= 1;
	if(s) sv->nav_new_subframe |= 1;

	sv->nav_valid_bits++;
	if(sv->nav_valid_bits == 32) {
		if(nav_valid_subframe(sv)) {
			sv->nav_valid_bits = 2;
		} else {
			sv->nav_valid_bits = 31;
		}
	}
	sv->nav_seq++;
}
...
static void nav_abandon(struct Space_vehicle *sv) {
	printf("%2i: Abandon - %5i errors\n", sv->id, sv->nav_bit_errors);
	sv->nav_valid_bits = 0;
	sv->nav_frame_synced = 0;
	/* Force drop if we havn't got a good bit after 'n' transistions */
	if(sv->nav_bit_errors > MAX_BAD_BIT_TRANSITIONS)
		sv->state = state_acquiring;
    sv->nav_bit_errors++;
}

static void nav_process(struct Space_vehicle *sv, uint_8 s) {
	if(sv->nav_part_in_bit == BIT_LENGTH-1) {
		if(sv->nav_bit_errors>0)
			sv->nav_bit_errors--;
		sv->nav_part_in_bit = 0;
	} else if(s != (sv->nav_new_subframe&1)) {
		nav_abandon(sv);
		sv->nav_part_in_bit = 0;
	} else
		sv->nav_part_in_bit++;
	
	if(sv->nav_part_in_bit == 0) {
		nav_new_bit(sv,s);
	}
}
...
static void start_locked(struct Space_vehicle *sv, int offset_from_peak) {
	...
	start_freq = (int)band_bandwidth * ((int)sv->track_band-(search_bands/2));
	lower_delta = (sv->track_power[1][1] - sv->track_power[1][0])/1024;
	upper_delta = (sv->track_power[1][1] - sv->track_power[1][2])/1024;
	...
	sv->lock_filtered_power = 1000000;
	...
	sv->lock_phase_nco_sine       = 0;
	sv->lock_phase_nco_cosine     = 0x40000000;
	sv->lock_phase_nco_step       = step;

	sv->lock_code_nco_step		  = (unsigned)((long long)(1<<22)*1023/samples_per_ms);
	sv->lock_code_nco_filter      = 0;
	sv->lock_code_nco = offset_from_peak * sv->lock_code_nco_step;
	sv->lock_code_nco_trend = start_freq*272/100;

	/* Set up the last angle, preventing any initial angle from
	   upsetting the ability lock */
	s = sv->lock_prompt_sine_total;
	c = sv->lock_prompt_cosine_total;	
	while(c > 15 || c < -15 || s > 15 || s < -15) {
		c /= 2;
		s /= 2;
	}
	s &= ATAN2_SIZE-1;  c &= ATAN2_SIZE-1;
	sv->lock_last_angle = atan2_lookup[c][s];

	/* Reset the running totals */
	sv->lock_early_sine_total     = -(int)(samples_per_ms-offset_from_peak)/4;
	sv->lock_early_cosine_total   = -(int)(samples_per_ms-offset_from_peak)/4;
	sv->lock_prompt_sine_total    = -(int)(samples_per_ms-offset_from_peak)/4;
	sv->lock_prompt_cosine_total  = -(int)(samples_per_ms-offset_from_peak)/4;
	sv->lock_late_sine_total      = -2; //(int)(samples_per_ms-offset_from_peak)/4;
	sv->lock_late_cosine_total    = -2; //(int)(samples_per_ms-offset_from_peak)/4;
	sv->lock_early_power = 0;
	sv->lock_early_power_not_reset = 0;
	sv->lock_prompt_power = 2 * lock_lost_power;
	sv->lock_late_power = 0;
	sv->lock_late_power_not_reset = 0;
	sv->lock_delta_filtered		= 0;
	sv->lock_angle_filtered     = 0;
	sv->lock_ms_of_bit          = 0;
	sv->nav_bit_errors          = 0;
	...
	printf("Freq guess %i\n",start_freq);
	printf("\nLock band %i, Lock offset %i, step %x,  Code NCO %8x\n", sv->track_band, sv->track_offset, (unsigned) step, sv->lock_code_nco>>22);
	printf("lock_phase_nco_step  %8x\n",sv->lock_phase_nco_step);
	printf("lock code_nco & step    %8x   %8x %i\n",sv->lock_code_nco, sv->lock_code_nco_step, sv->lock_code_nco_trend);
#endif
...
static void start_tracking(struct Space_vehicle *sv) {
	...
	sv->percent_locked = 50;

	/* Where we are hunting around */
	sv->track_band       = sv->acquire_max_band;
	sv->track_offset     = sv->acquire_max_offset;

	sv->track_max_power  = 0;		   
}
...
void track_update(struct Space_vehicle *sv) {
	...
	**********************************************************/
	if(sv->track_max_power < track_unlock_power) {
		sv->percent_locked--;
		if(sv->percent_locked==0) {
			start_acquire(sv);
#if SHOW_LOCK_UNLOCK		 
			printf("Unlocked %i\n", sv->id);
#endif
		...
		***************************************/
		if(sv->track_power[1][1] >= sv->track_power[1][0] &&
           sv->track_power[1][1] >= sv->track_power[1][2] &&
           sv->track_power[1][1] >= sv->track_power[0][1] &&
           sv->track_power[1][1] >= sv->track_power[2][1]) {
            sv->percent_locked+=5;
		}

        if(sv->percent_locked > 99) {
			sv->percent_locked = 100;  
            start_locked(sv, 2);  // Offset of two from peak power
        } else {
			sv->track_max_power = 0;
			sv->track_band   = sv->track_max_band;
			sv->track_offset = sv->track_max_offset;
		}
    }
}
/*********************************************************************
...
void track(struct Space_vehicle *sv) {
	int line,i;
	if((sv->track_offset+1)%samples_per_ms == code_offset_in_ms) {
	    line =0;
	} else if(sv->track_offset == code_offset_in_ms) {
		line = 1;
	} else if((sv->track_offset+samples_per_ms-1)%samples_per_ms == code_offset_in_ms) {
		line = 2;
	} else if((sv->track_offset+samples_per_ms-2)%samples_per_ms == code_offset_in_ms) {
		track_update(sv);
		return;
	} else {
		return;
    ...
    if(line == 0) printf("\n");
    printf("%2i %2i%% %5i: ", sv->id, sv->percent_locked, code_offset_in_ms);
	for(i = 0; i < sv->track_band-1;i++)
			printf("      ");      
#endif
	for(i = 0; i <3; i++) {
		int band;
		...
		band = sv->track_band-1+i;
		if(band < 0) band = 0;
		if(band >= search_bands) band = search_bands-1;

		xor_to_work(work_buffer, sample_history, sv->seek_in_phase[band]);
		sin_power = count_one_bits(work_buffer, samples_for_acquire) - samples_for_acquire/2;

		xor_to_work(work_buffer, sample_history, sv->seek_quadrature[band]);
		...
		sv->track_power[line][i] = power;
#if PRINT_TRACK_POWER
		if(power > PRINT_TRACK_SQUETCH) 
			printf("%5i ", sv->track_power[line][i]/1000);
		else
			printf("      ");      
#endif
		if(power > sv->track_max_power) {
			sv->track_max_power  = power;
			sv->track_max_band   = band;
			sv->track_max_offset = code_offset_in_ms;
		}
	}
#if PRINT_TRACK_POWER
	printf("\n");      
...
static void acquire(struct Space_vehicle *sv) {
...
#endif
      if(sv->acquire_max_power < power) {
        sv->acquire_max_power  = power;
        sv->acquire_max_band   = band;
        sv->acquire_max_offset = code_offset_in_ms;
      }
   }
#if PRINT_ACQUIRE_POWERS
   printf("\n");
   ...
   if(code_offset_in_ms  == 0) {
	   if(sv->acquire_max_power > acquire_min_power) {
		   start_tracking(sv);
	   } else {
		  sv->acquire_max_power = 0;		   
	   }
	   printf("%02i: Max power %7i at band %2i, offset %5i %s\n", 
		  	sv->id,
			sv->acquire_max_power,
			sv->acquire_max_band,
			sv->acquire_max_offset,
...
static void adjust_early_late(struct Space_vehicle *sv) {
	int adjust =  sv->lock_code_nco_trend;
	/* Use the relative power levels of the late and early codes 
	 * to adjust the code NCO phasing */
	if(sv->lock_early_power/5 > sv->lock_late_power/4) {
		sv->lock_late_power = (sv->lock_early_power*3+sv->lock_late_power)/4;
		adjust += samples_per_ms*2;
		sv->lock_code_nco_trend+=8;
	} else if(sv->lock_late_power/5 > sv->lock_early_power/4) {
		sv->lock_early_power = (sv->lock_late_power*3+sv->lock_early_power)/4;
		adjust  -= samples_per_ms*2;
		sv->lock_code_nco_trend-=8;
	}

	if(adjust < 0) {
		if( sv->lock_code_nco  < -adjust) 
			sv->lock_code_nco += 1023<<22;
	}
	sv->lock_code_nco += adjust;

	if(sv->lock_code_nco+adjust >= 1023<<22) 
		sv->lock_code_nco -= 1023<<22;
...
static void update_early_late(struct Space_vehicle *sv, int sample) {
    ...
    prompt_code_index            = sv->lock_code_nco>>22;
	prompt_code_index_next_cycle = (sv->lock_code_nco + sv->lock_code_nco_step)>>22;
	...
	***********************************************************/
	sine_positive   = (sv->lock_phase_nco_sine   & 0x80000000) ? 0 : 1;
	cosine_positive = (sv->lock_phase_nco_cosine & 0x80000000) ? 0 : 1;
	
	if(sv->gold_code[early_code_index] ^ sample) {
		if(sine_positive)   sv->lock_early_sine_total++;
		if(cosine_positive) sv->lock_early_cosine_total++;
	}

	if(sv->gold_code[late_code_index] ^ sample) {
		if(sine_positive)   sv->lock_late_sine_total++;
		if(cosine_positive) sv->lock_late_cosine_total++;
	}
	...
	***********************************************************/
	if(early_code_index == CHIPS_PER_MS-1) {
		sv->lock_early_power -= sv->lock_early_power/LATE_EARLY_IIR_FACTOR;
		sv->lock_early_power += sv->lock_early_sine_total   * sv->lock_early_sine_total
                       		 + sv->lock_early_cosine_total * sv->lock_early_cosine_total;
							 
		sv->lock_early_power_not_reset -= sv->lock_early_power_not_reset/LATE_EARLY_IIR_FACTOR;
		sv->lock_early_power_not_reset += sv->lock_early_sine_total   * sv->lock_early_sine_total
                       		 + sv->lock_early_cosine_total * sv->lock_early_cosine_total;
		sv->lock_early_sine_total   = 0 - samples_per_ms/4;
		sv->lock_early_cosine_total = 0 - samples_per_ms/4;
	} else 	if(late_code_index == CHIPS_PER_MS-1) {
		sv->lock_late_power -= sv->lock_late_power/LATE_EARLY_IIR_FACTOR;
		
		sv->lock_late_power += sv->lock_late_sine_total   * sv->lock_late_sine_total
                     		+ sv->lock_late_cosine_total * sv->lock_late_cosine_total;
							
		sv->lock_late_power_not_reset -= sv->lock_late_power_not_reset/LATE_EARLY_IIR_FACTOR;		
		sv->lock_late_power_not_reset += sv->lock_late_sine_total   * sv->lock_late_sine_total
                              		   + sv->lock_late_cosine_total * sv->lock_late_cosine_total;
							
		sv->lock_late_sine_total    = 0 - samples_per_ms/4;
		sv->lock_late_cosine_total  = 0 - samples_per_ms/4;
...
static void adjust_prompt(struct Space_vehicle *sv) {
	int s = sv->lock_prompt_sine_power;
	int c = sv->lock_prompt_cosine_power;
	int adjust = 0;
	uint_8 angle;
	int_8 delta;
	uint_8 this_bit;
	...
	printf("%2i-%4i:  %6i, %6i\n", sv->id, sv->lock_code_nco>>22, 
	sv->lock_prompt_sine_power, sv->lock_prompt_cosine_power);
#endif		
	while(c > 15 || c < -15 || s > 15 || s < -15) {
		c /= 2;
		s /= 2;
	...
	delta = angle - sv->lock_last_angle;
	sv->lock_last_angle = angle;
	sv->lock_delta_filtered -= sv->lock_delta_filtered / LOCK_DELTA_IIR_FACTOR;
	sv->lock_delta_filtered += delta;
	adjust = angle;
	sv->lock_angle_filtered -= sv->lock_angle_filtered / LOCK_ANGLE_IIR_FACTOR;
  
	sv->lock_angle_filtered += angle;
	if(angle >=128)
		sv->lock_angle_filtered -= 256;

	adjust = sv->lock_angle_filtered/8;
	
	adjust  += (1<<24) / 32 / LOCK_DELTA_IIR_FACTOR / samples_per_ms * sv->lock_delta_filtered;
	sv->lock_phase_nco_step  -= adjust;
#if LOCK_SHOW_ANGLES
	printf("%6i, %6i, %3i,%4i,%6i, %6i, %6i\n",sv->lock_prompt_sine_power, sv->lock_prompt_cosine_power, angle,delta, sv->lock_delta_filtered, adjust, sv->lock_phase_nco_step);
#endif	
  
	if(sv->lock_prompt_cosine_power < 0)
		this_bit = 0;
	else
		this_bit = 1;
	sv->lock_ms_of_bit++;
	if(sv->lock_ms_of_bit == MS_PER_BIT || sv->lock_last_bit != this_bit)
		sv->lock_ms_of_bit = 0;
	sv->lock_last_bit = this_bit;

	if(sv->lock_ms_of_frame == MS_PER_BIT*BITS_PER_FRAME-1) {
		sv->lock_ms_of_frame = 0;
		if(sv->nav_frame_of_week == 7*24*60*60/6-1) {
			sv->nav_frame_of_week=0;
		} else {
			sv->nav_frame_of_week++;
		}
	} else {
		sv->lock_ms_of_frame++;
	}
...
static void update_prompt(struct Space_vehicle *sv, int sample) {
	...
	***********************************************************/
    ...
    if(sv->gold_code[prompt_code_index] ^ sample) {
		if(sine_positive)   sv->lock_prompt_sine_total++;
		if(cosine_positive) sv->lock_prompt_cosine_total++;
	}
	...
	***********************/
	sv->lock_prompt_sine_power   = sv->lock_prompt_sine_total;
	sv->lock_prompt_cosine_power = sv->lock_prompt_cosine_total;
	
	sv->lock_prompt_power -= sv->lock_prompt_power/LATE_EARLY_IIR_FACTOR;
	sv->lock_prompt_power += sv->lock_prompt_sine_total   * sv->lock_prompt_sine_total
						   + sv->lock_prompt_cosine_total * sv->lock_prompt_cosine_total;
	if(sv->lock_prompt_power/LATE_EARLY_IIR_FACTOR < lock_lost_power) { 
	    printf("Lock lost at power %u", sv->lock_prompt_power/LATE_EARLY_IIR_FACTOR);
		sv->state = state_acquiring;
		return;
	}
	sv->lock_prompt_sine_total   = 0 - samples_per_ms/4;
	sv->lock_prompt_cosine_total = 0 - samples_per_ms/4;
	adjust_prompt(sv);
}
...
static void update_ncos(struct Space_vehicle *sv) {	
	...
	***************************/
	sv->lock_phase_nco_sine   += sv->lock_phase_nco_step;
	sv->lock_phase_nco_cosine += sv->lock_phase_nco_step;
	/***********************************
	* Update the current C/A code offset
	***********************************/
	sv->lock_code_nco += sv->lock_code_nco_step;
    if(sv->lock_code_nco >= CHIPS_PER_MS<<22)
		sv->lock_code_nco -= CHIPS_PER_MS<<22;
}
...
static void snapshot_timing(struct Snapshot *s) {
	...
	if(snapshot_file != NULL) {
		for(i = 0; i < N_SV; i++) {
			...
			s->entries[i].nav_frame_of_week     = sv->nav_frame_of_week;
			s->entries[i].lock_ms_of_frame      = sv->lock_ms_of_frame;
			s->entries[i].lock_code_nco         = sv->lock_code_nco;
			s->entries[i].nav_subframe_in_frame = sv->nav_subframe_in_frame;
			s->entries[i].nav_valid_bits        = sv->nav_valid_bits;
			s->entries[i].lock_ms_of_bit        = sv->lock_ms_of_bit;
		}
	...
	for(i = 0; i < N_SV; i++) {
		...
		printf(" %6i,      %6i,      %4i,       %4i,         %2i %1i  %2i %2i %c%c%c%c%c\n",
			...
			sv->lock_ms_of_frame,
			(sv->lock_code_nco>>22),
			(sv->lock_code_nco>>16)&0x3f,
			sv->nav_subframe_in_frame,
			sv->nav_valid_bits-2,
			sv->lock_ms_of_bit,
			(sv->nav_valid_frame[1] ? '1' : '.'),
			(sv->nav_valid_frame[2] ? '2' : '.'),
			(sv->nav_valid_frame[3] ? '3' : '.'),
			(sv->nav_valid_frame[4] ? '4' : '.'),
			(sv->nav_valid_frame[5] ? '5' : '.')
			);
	}
	printf("\n");
#endif
...
static void stretchGoldCodes(void) {
  ...
  for(i = 0; i < N_SV; i++) {
	...
	for(j = 0; j < samples_for_acquire; j++) {
	   ...
	   bitmap_set_bit(space_vehicles[i].gold_code_stretched, 
					  samples_for_acquire-1 - j,
	                  space_vehicles[i].gold_code[index]);
//	   add_to_bitmap(space_vehicles[i].gold_code_stretched, space_vehicles[i].gold_code[index]);
	}
  }
}
  ...
  for(band = 0; band < search_bands; band++) {		
    ...
    **********************************/
    ...
    for(i = 0; i < N_SV; i++) {
	   struct Space_vehicle *sv;
  	   sv = space_vehicles+i;
	   xor_to_work(sv->seek_in_phase[band], work_buffer, sv->gold_code_stretched);
#if 0
           printf("\n\n");
    ...
    **********************************/
    ...
    for(i = 0; i < N_SV; i++) {
	   struct Space_vehicle *sv;
	   sv = space_vehicles+i;
	   xor_to_work(sv->seek_quadrature[band], work_buffer, sv->gold_code_stretched);
    }
  }
}
...
int gps_setup(int sample_rate, int if_freq) {
  ...
  *************************************************/
  ...
  for(i = 0; i < N_SV; i++) {
	...
	sv->gold_code_stretched = malloc(acquire_bitmap_size_u32*4);
	if(sv->gold_code_stretched == NULL) {
		printf("Out of memory for gold_code_stretched\n");
		return 0;
	}

        sv->seek_in_phase       = malloc(sizeof(uint_32 *)*search_bands);
	if(sv->seek_in_phase == NULL) {
		printf("Out of memory for seek_in_phase\n");
		return 0;
	}

    sv->seek_quadrature     = malloc(sizeof(uint_32 *)*search_bands);
	if(sv->seek_quadrature == NULL) {
		printf("Out of memory for seek_quadrature\n");
		return 0;
	}

	for(band = 0; band < search_bands; band++) {		
		sv->seek_in_phase[band]            = malloc(acquire_bitmap_size_u32*4);
		if(sv->seek_in_phase[band]==NULL) {
			printf("Out of memory for sv->seek_in_phase[]\n");
			return 0;			
		}

		sv->seek_quadrature[band] = malloc(acquire_bitmap_size_u32*4);
		if(sv->seek_quadrature[band]==NULL) {
...
int main(int argc, char *argv[]) {
  ...
  sprintf(snapshot_filename,"snapshots_%i.dat",time(NULL));
